...
describe("cli/create", () => {
        ...
        it("returns {exists:false, shouldProceed:true} if dir missing", async () => {
            await withTempDir(async (cwd) => {
                ...
                try {
                    ...
                    const res = await checkDirectoryExists(target);
    ...
    describe("selectRuntime", () => {
        it('returns "node" in non-TTY environment (default)', async () => {
            ...
            try {
                // Dynamically import to ensure a fresh module instance for testing
        ...
        it("creates a new project successfully", async () => {
            await withTempDir(async (cwd) => {
                ...
                try {
                    ...
                    await handleCreateCommand("my-app", {});
        ...
        it("cancels if directory exists and user declines", async () => {
            await withTempDir(async (cwd) => {
                ...
                try {
                    ...
                    await handleCreateCommand("exists", {});
        ...
        it("handles empty template list from GitHub", async () => {
            await withTempDir(async (cwd) => {
                ...
                try {
                    ...
                    await handleCreateCommand("empty-repo", {});
                    assert.equal(process.exitCode, 1);
        ...
        it("handles GitHub API 404 for templates directory", async () => {
            await withTempDir(async (cwd) => {
                ...
                try {
                    ...
                    await handleCreateCommand("missing-templates", {});
                    assert.equal(process.exitCode, 1);
        ...
        it("warns if GitHub tree is truncated", async () => {
            await withTempDir(async (cwd) => {
                ...
                try {
                    ...
                    await handleCreateCommand("truncated", {});
        ...
        it("handles GitHub API error (List)", async () => {
            await withTempDir(async (cwd) => {
                ...
                try {
                    ...
                    await handleCreateCommand("api-error", {});
                    assert.equal(process.exitCode, 1);
        ...
        it("handles GitHub API error (File Download)", async () => {
            await withTempDir(async (cwd) => {
                ...
                try {
                    ...
                    await handleCreateCommand("file-error", {});
                    assert.equal(process.exitCode, 1);
            ...
            try {
                await withTempDir(async (cwd) => {
                    ...
                    try {
                        ...
                        await handleCreateCommand("overwrite-me", {});
        ...
        it("handles multiple project_name occurrences in text files (interpolation)", async () => {
            await withTempDir(async (cwd) => {
                ...
                try {
                    ...
                    await handleCreateCommand("my-project", {});
        ...
        it("handles text files with no project_name placeholder", async () => {
            await withTempDir(async (cwd) => {
                ...
                try {
                    ...
                    await handleCreateCommand("any-app", {});
        ...
        it("detects binary files with null bytes and writes them as-is (no interpolation)", async () => {
            await withTempDir(async (cwd) => {
                ...
                try {
                    ...
                    await handleCreateCommand("binary-app", {});
        ...
        it("batches downloads with concurrency limit (5 files per batch)", async () => {
            await withTempDir(async (cwd) => {
                ...
                try {
                    ...
                    await handleCreateCommand("batch-app", {});
            ...
            await withTempDir(async (cwd) => {
                ...
                try {
                    ...
                    try {
                        ...
                        await handleCreateCommand("progress-app", {});
        ...
        it("handles fetch timeout (AbortController signal)", async () => {
            await withTempDir(async (cwd) => {
                ...
                try {
                    ...
                    await handleCreateCommand("timeout-app", {});
                    assert.equal(process.exitCode, 1);
        ...
        it("handles special characters in project names (interpolation is exact string match)", async () => {
            await withTempDir(async (cwd) => {
                ...
                try {
                    ...
                    await handleCreateCommand(specialName, {});
        ...
        it("handles GitHub API server error during list (5xx response)", async () => {
            await withTempDir(async (cwd) => {
                ...
                try {
                    ...
                    await handleCreateCommand("server-error-app", {});
                    assert.equal(process.exitCode, 1);
        ...
        it("creates nested directory structure for files in subdirectories", async () => {
            await withTempDir(async (cwd) => {
                ...
                try {
                    ...
                    await handleCreateCommand("nested-app", {});
        ...
        it("logs next steps with relative path when creating in non-current directory", async () => {
            await withTempDir(async (cwd) => {
                ...
                try {
                    ...
                    await handleCreateCommand("new-app", {});

                    // Should have logged "cd new-app" as a next step (not shown for ".")
                    const textCalls = logMocks.text.mock.calls.map((c) => c.arguments[0]).filter((msg) => msg?.includes("cd "));
                    assert.equal(textCalls.length > 0, true);
                    assert.equal(
                        textCalls.some((msg) => msg.includes("cd new-app")),
                        true,
                    );
                } finally {
                    process.chdir(originalCwd);
            ...
            try {
                await withTempDir(async (cwd) => {
                    ...
                    try {
                        ...
                        assert.equal(
                            textCalls.some((msg) => msg.includes('cd "."')),
                            false,
                        );
                        assert.equal(
                            textCalls.some((msg) => msg.match(/^ {2}cd /)),
                            false,
                        );
                    } finally {
                        process.chdir(originalCwd);
